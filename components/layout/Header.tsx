...
/**
 * El único componente de cliente del sitio. Necesita JS por tres cosas y ninguna más:
 * menú móvil, estado de scroll y saber en qué ruta estamos para el cambio de idioma.
 */
export function Header({ locale, dictionary }: Props) {
  ...
  return (
    <>
      <header
        // Con el menú abierto la barra deja de estar "sobre el hero": pasa a fondo claro y
        // grafito, para que se lea junto al panel desplegado.
        data-top={!scrolled && !open}
        className={cn(
          'sticky top-0 z-50 transition-colors duration-500',
          scrolled || open ? 'bg-metal/95 text-graphite backdrop-blur-md' : 'bg-transparent',
        )}
      >
        <a
          href="#main"
          className="sr-only focus:not-sr-only focus:absolute focus:top-3 focus:left-3 focus:z-50 focus:bg-graphite focus:px-3 focus:py-2 focus:text-metal"
        >
            ...
            {navigation.map((key) => {
              ...
              return (
                <Link
                  ...
                  // metal, sobre fondo claro es grafito. La jerarquía va con opacidad.
                  className={cn(
            ...
            <div className="flex items-center gap-2">
              {locales.map((option) => (
                <Link
                  ...
                  className={cn(
                    'text-micro tap font-mono uppercase transition-opacity',
      ...
      {/* Menú móvil: pantalla completa, tipografía grande, sin adornos.
          ...
          convierte al elemento en bloque contenedor de sus descendientes `fixed`. Dentro
          del header, este panel calcularía su alto contra una barra de 80 px y se quedaría
          en 0 px — el menú se abriría vacío. */}
      <div
        id="mobile-nav"
        hidden={!open}
        className="page-gutter fixed inset-0 top-20 z-40 bg-metal md:hidden"
      >
          ...
          <div className="mt-6 flex items-center gap-5 border-t border-line pt-6">
            {locales.map((option) => (
              <Link
                ...
                href={swapLocale(option)}
                hrefLang={option}
                className={cn('eyebrow tap', option === locale && 'text-graphite')}
              >
                {localeNames[option]}
              </Link>
            ))}
